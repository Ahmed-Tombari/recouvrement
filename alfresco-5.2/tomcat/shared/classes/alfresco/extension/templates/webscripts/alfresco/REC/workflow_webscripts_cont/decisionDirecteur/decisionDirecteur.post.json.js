...
if(action == "petit montant"){
	
    ws.getTaskService().setVariable(taskId,"drct_decisionDirecteurOutcome",action);
	...
	message = "Petit Montant"; 
}
else{

if(action == "grand montant"){
	
    ws.getTaskService().setVariable(taskId,"drct_decisionDirecteurOutcome",action);
    wsbean.endTask("activiti$"+taskId,"Next");
    result = ws.getTaskService().createTaskQuery().processInstanceId(instanceWorkflow).orderByTaskId().asc().list();
    message = "Grand Montant";
  }
}
